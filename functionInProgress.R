...
astrain$event <- astrain$event != 0
data.table::setorder(astrain, time, -event)
astrainX <- model.matrix(event ~ . - time, data = astrain)[, -c(1)]
astrainY <- data.matrix(astrain[, c(9, 8)])

newData <- simActiveSurveillance(208)
newData$event <- newData$event != 0
data.table::setorder(newData, time, -event)
newDataX <- model.matrix(event ~ . - time, data = newData)[, -c(1)]
newDataY <- data.matrix(newData[, c(9, 8)])
...
Err <- brierScoreKMCens(predSurvs = absRiskcb, times = times, newData = newData)
...
legend(x = "topright", legend = c("cox", "cbGompertz"), col = c("red", "black"))
title("RED IS COX")
...
results <- data.frame(riskRegression = X2$Brier$score$Brier[X2$Brier$score$model == "PredictionModel"], cbBrier = Err, times = times)
...
ggplot(data = results, mapping = aes(x = times, y = value, col = variable)) +
  ...
  xlab("Times") +
  ylab("Brier-Score")+
  ggtitle("cbBrier using KM censoring probabilites")


Err2<-brierScoreweibCens(predSurvs=absRiskcb, times, newDataX,newDataY)
resultsCB<-rbind(rbind(data.frame(brierScore=Err,times=times,model="KMCensoring")),rbind(data.frame(brierScore=Err2,times=times,model="WeibullCensoring")),data.frame(brierScore = X2$Brier$score$Brier[X2$Brier$score$model == "PredictionModel"],times=times,model="riskRegressionCOX"))
